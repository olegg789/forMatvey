...
function MiddleNotes({middleNotes, router, isDesktop, editNote, openSnackbar, allNotes, getNotes, openSnackbarDel, scheme, setPopout, platform, offline}) {
    ...
    return (
        <>
            {middleNotes.items.map((el, index) => {
                return (
                    <Div>
                        <Card mode='outline'>
                            <FormLayout>
            ...
            <Footer>{middleNotes.count !== 0 ?
                `Всего ${middleNotes.count} ${declOfNum(middleNotes.count, ['заметка', 'заметки', 'заметок'])}`
                :
                'Тут нет заметок'
            }</Footer>
            {snackbarDel}
